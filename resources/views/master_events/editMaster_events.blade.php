...
@extends("../navbar")
    <div class="p-5">
        <h1 class="font-bold text-3xl mb-4">Edit Events</h1>
        <form action="{{ route('master_events.update', $events->id) }}" method="POST" class="flex flex-col gap-4">
            @csrf
            @method('PUT')
            <div>
                <p class="font-bold text-lg mb-1">{{ $events->title }}</p>
                <input type="text" name="title" class="outline outline-1 w-full p-2" required placeholder="Events Name">
            </div>

            <div>
                <p class="font-bold text-lg mb-1">{{ $events->date }}</p>
